...
import { Flame, PanelLeft, Utensils } from 'lucide-react';
...
function NavLinks({
  ...
  const resolvedItems = React.useMemo(() => {
    // Menu is only available for Restaurants, Cafes, and Bakeries.
    // We add it dynamically so it cannot accidentally appear for other categories.
    if (isMenuUploadEnabled(storeCategory)) {
      return [
        ...items.slice(0, 3),
            ...
            <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
              <ul className="space-y-1">
                <NavLinks
                  ...
                  role={session?.isAuthenticated ? session.role : undefined}
                  storeCategory={session?.isAuthenticated ? session.storeCategory : null}
                />
              </ul>
            </nav>
          </div>
